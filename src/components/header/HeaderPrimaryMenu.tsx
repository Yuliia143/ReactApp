...
const HeaderPrimaryMenu = ({ user, onSignOut }: PropsFromRedux) => {
    ...
    return (
        ...
        >
            <Dropdown.Menu
                direction="left"
                style={{ marginTop: 0, width: '200px', maxHeight: '200px' }}
            >
                ...
                {user && user.role === 'student' ? (
                    <Dropdown.Item
                        as={Link}
                        name="favorite"
                        to="/favorite-lections"
                    >
                        Favorites
                    </Dropdown.Item>
                ) : null}
                {user && user.role === 'admin' ? (
                    <Dropdown.Item as={Link} name="admin" to="/admin">
                        Admin page
                    </Dropdown.Item>
                ) : null}
                ...
                <Dropdown.Item as={Link} name="webinars" to="/webinar/all">
                    Webinars
                </Dropdown.Item>
                <Dropdown.Item
                    as={Link}
